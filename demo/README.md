...
3. That schema (and the engine's dialect) is posted to the worker (`src/lib/worker.ts`), which feeds it to the querent `EngineProvider` keyed by the document URI.
