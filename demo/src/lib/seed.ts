...
/**
 * Equivalent dataset for PGlite (real Postgres in wasm). Mirrors the DuckDB
 * schema with Postgres syntax: `generate_series` for row generation, quoted
 * `INTERVAL` literals, and a genuine BOOLEAN `in_stock`.
 */
export const PGLITE_SEED = `
CREATE TABLE customers (
  id         INTEGER PRIMARY KEY,
  name       VARCHAR,
  email      VARCHAR,
...
/** See {@link SQLITE_DEFAULT}; Postgres filters the BOOLEAN `in_stock` directly. */
export const PGLITE_DEFAULT = `
-- select
SELECT * FROM products;

-- select, filter, order limit
SELECT name, price FROM products WHERE in_stock ORDER BY price DESC LIMIT 3;
