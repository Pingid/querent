...
pub fn supports(ctx: &context::Context) -> bool {
    if matches!(ctx.cursor.location, context::Location::Start) {
        return true;
    }
    if matches!(ctx.cursor.location, context::Location::Ident) {
        return true;
    }
    // Space after ident/keyword suggests next keywords
    if let context::Location::Space(inner) = &ctx.cursor.location {
        match **inner {
            context::Location::Ident | context::Location::Keyword => return true,
...
pub fn complete(ctx: &context::Context, spec: &DialectSpec) -> Vec<Completion> {
    ...
    let after_or_on_keyword = match &ctx.cursor.location {
        ...
        context::Location::Space(inner) => matches!(**inner, context::Location::Keyword),
        _ => false,
    };
    if after_or_on_keyword {
    ...
    } else if let context::Location::Space(inner) = &ctx.cursor.location {
        ...
        if matches!(**inner, context::Location::Literal) {
                ...
                || ctx.cursor.preceding.contains(&Keyword::Then)
            {
                ...
                if let Some(last_relevant) = ctx
                    .cursor
                    .preceding
                    .iter()
                    .rev()
                    .find(|&&kw| matches!(kw, Keyword::Case | Keyword::When | Keyword::Then))
                {
